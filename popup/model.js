...
class Model {
    ...
    async requestPermissions() {
        await browser.permissions.request({ origins: ["*://dogechain.info/*", doginalsPermissionsUri, "*://api.blockchair.com/*"] })
    ...
    async refreshUtxos() {
        ...
        if (resp3.status != 200) throw new Error("doginals endpoint is out of sync")
    ...
    async refreshInscriptionIds() {
        ...
        for (const utxo of this.utxos) {
            ...
            if (!inscriptionIdsPerOutput[key]) {
                const resp = await fetch(`${doginalsUri}/output/${utxo.txid}:${utxo.vout}`)
